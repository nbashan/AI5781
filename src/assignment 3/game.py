...
def create(s):
    ...
    # return [board, 0.00001, playTurn, r*c]
    # 0 is TIE
...
# sum of all heuristic value of alpha beta pruning in matrix
def value(s):
...
def checkSeq(s, r1, c1, r2, c2):
    ...
    if sum == COMPUTER * SIZE:
        ...
        # we found that if we do it with an exponential function with extreme point at 9.5
...
# gets a boars and a column and puts the chip in the right column
def makeMove(s, c):
...
def getNext(s):
    ...
    for c in list(range(columns)):
        # an empty cell
        if s.board[0][c] == 0:
            # make a deep copy of the board and build one step further
            tmp = cpy(s)
            makeMove(tmp, c)
            # add this state to the list
            ns += [tmp]
            # list of the next steps from the state we received at the beginning of the function
    return ns
